...
sys.path.insert(0,'../../../smart_door_ip_camera_monitoring')
    ...
    if(len(data)):
        ...
        if (d != None):
            ...
            #     client.publish(AIO_DOORMONITOR[1], d[1])
                ...
                if (d[1] == room_id): # Suppose we only consider ROOM0 and want to publish it
...
def on_detection_result_callback(results):
    for result in results:
        send_data = RoomMonitorData()
        send_data.ROOM = result.roomId
        send_data.HEAD_COUNT = result.count
        send_data.IMAGE_URL = result.camImgUrl
        json_str = json.dumps(send_data.__dict__)
        client.publish(AIO_DOORMONITOR[3], json_str) 

start_detection(on_detection_result_callback)
