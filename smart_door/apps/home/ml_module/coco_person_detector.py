...
def get_people_coco(image_path):
    ...
    image_with_boxes = draw_boxes(
      img.numpy(), np.array([p_box[i] for i in nms]),
      [p_class[i] for i in nms], [p_score[i] for i in nms])
    detection_image_path = os.path.splitext(image_path)[0] + '_coco.jpg'
    Image.fromarray(image_with_boxes).save(detection_image_path)
    return nms.shape[0], detection_image_path